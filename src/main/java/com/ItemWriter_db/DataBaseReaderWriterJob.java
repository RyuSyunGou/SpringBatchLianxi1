...
public class DataBaseReaderWriterJob {
    @Autowired
...
//                .resource(new PathResource("C:/IDEA/outUser.json"))//输出到DB
...
//    }
    ...
    public FlatFileItemReader<User> itemReader() {
        return new FlatFileItemReaderBuilder<User>()
                .name("userItemReader")
                //获取文件或数据/资源
                .resource(new ClassPathResource("user_db.txt"))
    ...
    @Bean
    public Job job() {
        return jobBuilderFactory.get("database-writer-job")
                .start(step())
                .build();
    }

    public static void main(String[] args) {
        SpringApplication.run(DataBaseReaderWriterJob.class, args);
    }
}
